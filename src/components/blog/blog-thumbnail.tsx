...
const BlogThumbnail = async ({ slug, title, image, date, views }: Props) => {
  return (
    <div
      data-test="blog-thumbnail"
      className="w-full cursor-pointer rounded-xl p-4 transition duration-200 hover:bg-white hover:shadow hover:ring-1 hover:ring-primary-200 dark:hover:bg-primary-900 dark:hover:ring-1 dark:hover:ring-primary-800 md:w-[calc(100%/2-1rem/2)] xl:w-[calc(100%/3-2*1rem/3)]"
    >
      <Link href={`/blogs/${slug}`} className="block h-full w-full">
        <div className="grid grid-cols-2 gap-4">
          ...
          <span
            data-test="blog-date"
            className="whitespace-nowrap text-lg dark:text-dark-medium"
          >
