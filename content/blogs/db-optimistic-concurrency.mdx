...
Imagine two users, Angelo and Bruno, who are both trying to book the same ticket. Here's how the race condition unfolds:
...
Notice that when an attempt is made to to reserve the ticket, the version is included in the query. Moreover, after a successful update, the version is incremented.
