...
description: Learn how to improve code quality, readability, and maintanability with the state machine pattern.
published: 2024-05-04
lastEdited: 2024-05-04
imageSrc: /blogs/state-machine/t3.jpg
tags:
  - Design Patterns
  - State Machines
---
...
However, if project requirements change (i.e. a new state is added), all the switch statements will need to be modified!
...
<ThemeImage
  ...
  height={424}
/>
The key takeaway from this diagram is that we encapsulate each state into its own
class, all inheriting from the base `State` class.
...
Note that the `Context` class stores a reference to one of the concrete `State` objects in order to delegate all state-specific work.
The `Context` also exposes a setter for passing it a new state object (i.e. when we need to perform a state transition).
...
public class SuperMario : MarioState
{
    ...
    public override void CollideEnemy()
    {
        ...
        mario.TransitionTo(new SmallMario(mario));
    }
}
```
...
public class DeadMario : MarioState
{
    public override void ObtainMushroom()
    {
        // Dead, so do nothing.
...
✔️ Code is organized into distinct state classes instead of conditionals, improving readability and maintanability.

✔️ The pattern allows for extensability (e.g. if we need to add a new state at some point later on, or make modifications to an existing state without impacting other states).
