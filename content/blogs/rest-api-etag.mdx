---
title: REST API Concurrency with ETag
description: Learn the importance of the ETag header in REST APIs to to prevent simultaneous updates of a resource from overwriting each other.
...
Prior to reading this article, I would recommend first reading my blog: [Optimistic Concurrency in Databases](/db-optimistic-concurrency).
...
Consider a blog website, where multiple users can make edits to the same blog post. Here is a simplified representation of the the REST API:
...
<Alert
  type="warning"
  text="A REST API without ETag headers are susceptible to overwriting changes made by another user!"
/>
...
<Alert
  ...
  text="The If-Match header is used in HTTP requests to ensure that a resource is updated only if it matches a specific version or state."
/>
...
As a result, User B will not override the changes made by User A!

<Alert
  type="correct"
  text="As a result, User B will not override the changes made by User A!"
/>
